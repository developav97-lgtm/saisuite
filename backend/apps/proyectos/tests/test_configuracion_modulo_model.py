"""
SaiSuite — Tests: ModuleSettings model
"""
import pytest
...
from apps.proyectos.models import ModuleSettings
...
class TestConfiguracionModuloModel:
    ...
    def test_requiere_sync_saiopen_se_puede_activar(self):
        ...
        config = ModuleSettings.objects.create(
    ...
    def test_dias_alerta_vencimiento_personalizado(self):
        ...
        config = ModuleSettings.objects.create(company=c, dias_alerta_vencimiento=30)
    ...
    def test_dias_alerta_vencimiento_rechaza_negativos(self):
        ...
        config = ModuleSettings(company=c, dias_alerta_vencimiento=-1)
        with pytest.raises(ValidationError):
            config.full_clean()

    def test_solo_una_configuracion_por_empresa(self):
        c = make_company('908001008')
        config1 = ModuleSettings.objects.create(company=c)
        assert ModuleSettings.objects.filter(company=c).count() == 1
        assert config1.company_id == c.id

    def test_get_or_create_devuelve_existente(self):
        c = make_company('908001009')
        config1, created1 = ModuleSettings.objects.get_or_create(company=c)
        assert created1 is True
        config2, created2 = ModuleSettings.objects.get_or_create(company=c)
        assert created2 is False
        assert config1.id == config2.id

    def test_get_or_create_crea_nueva_si_no_existe(self):
        c = make_company('908001010')
        assert ModuleSettings.objects.filter(company=c).count() == 0
        config, created = ModuleSettings.objects.get_or_create(company=c)
    ...
    def test_diferentes_empresas_tienen_configuraciones_independientes(self):
        ...
        config1 = ModuleSettings.objects.create(
            company=c1, dias_alerta_vencimiento=7
        )
        config2 = ModuleSettings.objects.create(
