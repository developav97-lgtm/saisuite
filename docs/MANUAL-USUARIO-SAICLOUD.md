...
| Módulo | Descripción |
|---|---|
| Dashboard | Resumen ejecutivo de todos sus proyectos |
| Proyectos | Gestión completa de proyectos, fases y tareas |
| Mis tareas | Vista personal de todas las tareas asignadas a usted |
| Terceros | Gestión de clientes y proveedores |
| Catálogo | Actividades del catálogo compartido |
| Configuración | Parámetros de la empresa y usuarios (solo administradores) |
...
### 4.2 Lista de proyectos (vista cards y lista)

Al ingresar al módulo Proyectos, verá la lista de todos los proyectos de su empresa. Dispone de dos vistas:

**Vista Cards (predeterminada):**
- Cada proyecto se muestra como una tarjeta con: nombre, código, tipo, estado, porcentaje de avance, cliente y fechas.
- La barra de progreso verde indica el avance físico calculado automáticamente desde las tareas.
- Colores de estado: gris (borrador), azul (planificado), verde (en ejecución), amarillo (suspendido), negro (cerrado), rojo (cancelado).

**Vista Lista:**
- Tabla con más detalle: columnas de código, nombre, cliente, gerente, fechas, presupuesto, avance y estado.
- Útil para comparar múltiples proyectos al mismo tiempo.
- Soporta ordenamiento por columna (haga clic en el encabezado).
...
> **Tip:** Use el ícono de filtro en la esquina superior derecha de la lista para activar el panel de filtros avanzados.

### 4.3 Detalle del proyecto (tabs disponibles)

Al hacer clic en un proyecto, accede a su vista de detalle. Esta vista tiene las siguientes pestañas (tabs):

| Tab | Contenido |
|---|---|
| Resumen | Información general, estado financiero AIU, métricas principales |
| Fases | Lista de fases del proyecto |
| Tareas | Lista completa de tareas (con filtro por fase) |
| Kanban | Vista de tareas en tablero Kanban |
| Gantt | Diagrama de Gantt del proyecto |
| Actividades | Actividades del catálogo asignadas a este proyecto |
| Recursos | Asignaciones de recursos y workload |
| Timesheets | Registro de horas del equipo |
| Analytics | Dashboard de métricas y gráficos |
| Scheduling | Auto-Schedule, baselines, restricciones y escenarios What-If |
| Presupuesto | Budget tracking, EVM y gastos |
...
2. Haga clic en el botón **Editar** (ícono de lápiz) en la esquina superior derecha.
...
3. Seleccione **Eliminar proyecto**.
4. En el diálogo de confirmación, escriba el nombre del proyecto y haga clic en **Confirmar eliminación**.
...
1. Ingrese al detalle del proyecto.
2. Haga clic en el tab **Tareas** o en el tab **Kanban**.
...
| Fase | Sí | A qué fase pertenece la tarea |
...
| Seguidores | No | Usuarios que reciben notificaciones |
| Prioridad | No | 1=Baja, 2=Normal, 3=Alta, 4=Urgente |
| Etiquetas (tags) | No | Tags de color para categorización visual |
...
| Horas estimadas | No | Estimado de horas de trabajo (aplica si modo=horas) |
| Cantidad objetivo | No | Cantidad planificada (aplica si modo=cantidad) |
| Es recurrente | No | Si la tarea se repite periódicamente |
| Frecuencia | No (solo si es recurrente) | Diaria, Semanal, Mensual |
...
### 6.3 Vista lista y Kanban con filtro de fase

**Vista lista:**
- Tabla con columnas: código, nombre, fase, responsable, prioridad, estado, fechas, avance.
- Se puede filtrar por: fase, estado, responsable, prioridad, etiqueta, vencidas, sin responsable.
- Se puede ordenar por cualquier columna.
- Se puede buscar por nombre o código.

**Vista Kanban:**
- Columnas por estado: **Por hacer | En progreso | En revisión | Bloqueada | Completada | Cancelada**.
- Cada tarjeta muestra: nombre, código, responsable (avatar), prioridad (color), etiquetas y fecha límite.
- Se puede filtrar por fase usando el selector de fase en la parte superior.
- Las tarjetas con fecha límite vencida se muestran con borde rojo.
...
1. Haga clic en el nombre de la tarea en la vista lista o en la tarjeta del Kanban.
2. Se abre el panel de detalle de la tarea.
3. Haga clic en el ícono de lápiz (editar) para abrir el formulario de edición.
4. Modifique los campos y guarde.

**Cambio rápido de estado (inline):**
- En la vista lista, haga clic en el chip de estado de la tarea para cambiarlo directamente sin abrir el formulario.
...
3. Seleccione **Eliminar tarea**.
4. Confirme en el diálogo.
...
**Zoom:**
- Use los botones `+` y `−` en la barra de herramientas del Gantt para hacer zoom in/out.
- También puede usar `Ctrl + Rueda del mouse` para zoom rápido.
- Niveles disponibles: Días → Semanas → Meses → Trimestres.

**Scroll:**
- Desplazamiento horizontal: rueda del mouse o arrastrar la barra de scroll inferior.
...
Los overlays son capas de información adicional que se superponen al Gantt para facilitar el análisis.

**Para activar un overlay:**
1. En la barra de herramientas del Gantt, haga clic en el botón **Overlays** (o el menú de capas).
2. Seleccione el overlay deseado. Puede activar varios a la vez.
...
**Vista de timesheets semanales:**

También puede registrar timesheets desde la vista de timesheet semanal:

1. En el sidebar, dentro del módulo Proyectos, busque **Timesheets**.
2. La vista muestra una grilla semanal: filas = tareas activas asignadas a usted; columnas = días de la semana.
3. Ingrese las horas directamente en la celda correspondiente a la tarea y el día.
4. Al terminar la semana, haga clic en **Guardar semana**.

> **Tip:** La vista semanal es más eficiente para equipos que registran tiempo al final del día o de la semana. Use el Timer si prefiere registrar en tiempo real.
...
1. Abra el detalle de la tarea.
2. Haga clic en la sección **Recursos** o en el botón **+ Asignar recurso**.
...
1. Ingrese al tab **Recursos** del proyecto o al módulo de Resource Management.
2. Haga clic en **Capacidades**.
3. Seleccione el usuario.
4. Haga clic en **+ Nueva capacidad**.
5. Complete:
...
1. En el módulo de Resource Management, haga clic en **Disponibilidad**.
2. Haga clic en **+ Registrar ausencia**.
...
La vista **Workload** muestra gráficamente la carga de trabajo de cada miembro del equipo en el tiempo.

**Para acceder:**
1. En el tab **Recursos** del proyecto, haga clic en **Carga de trabajo**.
2. Se muestra una tabla con:
   - Filas: miembros del equipo.
   - Columnas: semanas del período seleccionado.
   - Celdas: porcentaje de utilización para esa semana.

**Código de colores:**

| Color | Utilización | Significado |
|---|---|---|
| Verde | 0% - 85% | Capacidad normal, sin conflictos |
| Amarillo | 86% - 100% | Carga alta, monitorear |
| Rojo | > 100% | Sobreasignado, requiere acción |
| Gris | Sin asignaciones | Recurso disponible |

#### Vista calendario por usuario

1. En el tab **Recursos**, haga clic en **Calendario del equipo**.
2. Seleccione un usuario en el filtro.
3. El calendario muestra:
   - Bloques de color por tarea asignada (con nombre del proyecto y tarea).
   - Franjas grises para ausencias aprobadas.
   - La densidad de bloques indica la carga de trabajo.
...
Los KPIs (indicadores clave de desempeño) se muestran en tarjetas en la parte superior del dashboard:

| KPI | Descripción |
|---|---|
| % de completud | Porcentaje de tareas completadas sobre el total de tareas del proyecto |
| Tasa a tiempo | Porcentaje de tareas completadas antes o en su fecha límite |
| Horas registradas | Total de horas de trabajo registradas en timesheets |
| Eficiencia | Relación entre horas estimadas y horas reales registradas |
| Tareas vencidas | Número de tareas con fecha límite pasada que no están completadas |
| Tareas activas | Tareas actualmente en estado "En progreso" |

> **Tip:** Un valor de eficiencia < 100% significa que se está usando más tiempo del estimado. > 100% significa que se completó con menos horas de lo previsto (alta eficiencia).
...
### 12.5 Exportación a Excel

Para exportar el reporte de analytics:

1. En el dashboard de analytics, haga clic en el botón **Exportar** (ícono de descarga).
2. Seleccione el formato: **Excel (.xlsx)**.
3. El archivo se descargará automáticamente con:
   - Hoja 1: KPIs del proyecto.
   - Hoja 2: Lista de tareas con estado y avance.
   - Hoja 3: Resumen de horas por usuario.
   - Hoja 4: Datos del Burn Down.
...
El módulo de **Advanced Scheduling** (Programación Avanzada) proporciona herramientas de nivel profesional para el cálculo y optimización del cronograma del proyecto.

**Para acceder:** Detalle del proyecto → tab **Scheduling**.
...
1. En el tab Scheduling, haga clic en **Auto-Schedule**.
2. Seleccione el modo (ASAP o ALAP).
3. Active la opción **Previsualizar cambios (dry run)**.
...
Después de ejecutar el Auto-Schedule, puede que algunos recursos queden sobreasignados en ciertos períodos (el algoritmo solo considera dependencias, no capacidad). La **nivelación de recursos** reprograma las tareas para eliminar o reducir la sobreasignación, manteniendo las dependencias intactas.

#### Cómo ejecutarla

1. En el tab Scheduling, haga clic en **Nivelar recursos**.
2. Active la opción **Previsualizar (dry run)** para ver el efecto antes de aplicar.
3. Haga clic en **Calcular**.
4. El sistema muestra:
   - Número de tareas que se moverían.
   - Reducción de sobreasignación (porcentaje máximo antes y después).
   - Si la nivelación fue efectiva o si el problema persiste.
5. Si está conforme, aplique los cambios.

> **Nota:** La nivelación puede extender la fecha de fin del proyecto. Es un compromiso entre terminar antes (sin nivelar) y no sobrecargar al equipo (con nivelación). Evalúe con el equipo cuál es la mejor opción.
...
1. En el detalle de la tarea, busque la sección **Restricciones de programación**.
2. Haga clic en **+ Agregar restricción**.
3. Seleccione el tipo de restricción.
4. Si el tipo requiere fecha, seleccione la fecha correspondiente.
5. Guarde.
...
1. En el tab Scheduling, haga clic en **Baselines**.
...
1. En el tab Scheduling → **Baselines**, haga clic en el nombre de una baseline.
2. Haga clic en **Comparar con avance actual**.
...
1. En la lista de baselines, haga clic en el botón **Establecer como activa** de la baseline deseada.
...
1. En el tab Scheduling → **Escenarios**, haga clic en **+ Nuevo escenario**.
2. Complete:
...
Las **tarifas de costo** definen cuánto cuesta por hora de trabajo cada miembro del equipo. Se usan para calcular el costo real de labor a partir de los timesheets.

**Para crear una tarifa:**

1. En el tab Presupuesto, haga clic en **Tarifas de recurso**.
2. Haga clic en **+ Nueva tarifa**.
3. Complete:
...
Los **gastos** registran todos los costos que no son horas de trabajo: materiales comprados, alquiler de equipos, viáticos, licencias de software, subcontratos, capacitaciones, etc.

#### Registrar un gasto

1. En el tab Presupuesto → sección **Gastos**, haga clic en **+ Registrar gasto**.
2. Complete el formulario:
...
El **Earned Value Management (EVM)** es la metodología estándar de la industria para medir el desempeño de un proyecto integrando alcance, tiempo y costo.

Para que el EVM funcione correctamente, el proyecto debe tener:
- Un presupuesto aprobado.
- Timesheets registrados.
...
Saicloud genera los datos para facturar al cliente, combinando horas de trabajo y gastos facturables.

**Para generar datos de factura:**

1. En el tab Presupuesto → sección **Facturación**, haga clic en **Generar datos de factura**.
2. El sistema calcula automáticamente:
   - **Líneas de labor:** para cada recurso, agrupa las horas registradas × su tarifa por hora.
   - **Líneas de gastos:** incluye todos los gastos marcados como "facturable" y aprobados.
3. Se muestra un resumen con:
   - Subtotal labor.
   - Subtotal gastos facturables.
   - Total general.
4. Haga clic en **Exportar** para descargar en Excel.

> **Nota:** Los datos de factura son informativos. Saicloud no genera facturas electrónicas directamente; los datos se exportan para ser procesados en el sistema de facturación (Saiopen u otro).
...
1. En el sidebar, haga clic en **Terceros**.
2. Haga clic en **+ Nuevo tercero**.
3. Complete el formulario:

| Campo | Obligatorio | Descripción |
|---|---|---|
| Nombre / Razón social | Sí | Nombre completo del tercero |
| Tipo | Sí | Cliente, Proveedor, o Ambos |
| NIT / Cédula | No | Número de identificación tributaria |
| Correo electrónico | No | Correo principal de contacto |
| Teléfono | No | Teléfono de contacto |
| Ciudad | No | Ciudad donde se ubica |
...
Al crear o editar un proyecto, el campo **Cliente** es un selector de terceros. Solo los terceros de tipo "Cliente" o "Ambos" aparecen en este selector.
...
8. Agregar restricciones SNET para tareas con fechas mínimas de inicio contractuales.
9. Ejecutar **Auto-Schedule ASAP** para calcular fechas.
10. Revisar el Gantt: identificar la ruta crítica.
11. Crear la **baseline inicial** (antes de iniciar obras).

**Paso 4: Recursos y costos**
12. Asignar recursos (ingenieros, maestros de obra) a las tareas.
13. Configurar tarifas de costo por recurso.
14. Crear el presupuesto de costos y aprobarlo.
...
20. Revisar el Gantt con overlay de ruta crítica: ¿alguna tarea crítica está retrasada?
21. Revisar el dashboard de Analytics: Burn Down y distribución de estados.
22. Revisar el EVM: CPI y SPI ≥ 0.95?
23. Tomar acciones correctivas si hay desviaciones.
24. Actualizar al cliente con los datos exportados del dashboard.
...
**Paso 3: Gestión ágil**
5. Usar el **Kanban** para el flujo diario: Todo → En progreso → En revisión → Completada.
6. Dependencias SS y FF para tareas paralelas (desarrollo de diferentes módulos en simultáneo).
7. Usar el **Timer** para registrar tiempo de desarrollo en tiempo real.

**Paso 4: Métricas**
8. Analytics: ver el **Velocity Chart** por sprint para detectar si el ritmo es sostenible.
9. Analytics: el **Burn Down** muestra si el equipo terminará a tiempo.
10. Crear baseline al inicio de cada sprint para comparar con el avance real del sprint.

**Paso 5: Facturación al cliente**
11. Marcar como "facturable" los gastos que se cobran al cliente.
12. Al cerrar el mes, generar los datos de factura desde el módulo de presupuesto.
13. Exportar y procesar en el sistema de facturación.
